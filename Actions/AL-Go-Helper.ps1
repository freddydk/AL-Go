...
function CommitFromNewFolder {
    ...
    if ($branch) {
        ...
        try {
            ...
            OutputError("GitHub actions are not allowed to create Pull Requests (see GitHub Actions Settings). You can create the PR manually by navigating to $($env:GITHUB_SERVER_URL)/$($env:GITHUB_REPOSITORY)/tree/$branch.")
        }
    }
    else {
